...
Namespace InventorAutoSaveAddIn
    <ProgIdAttribute("InventorAutoSaveAddIn.StandardAddInServer"), _
    GuidAttribute("9f5c012c-af13-469c-a083-77056c5e5e70")> _
    Public Class StandardAddInServer
        Implements Inventor.ApplicationAddInServer

        Private WithEvents m_timer As Timer
        Private WithEvents m_settingsButton As ButtonDefinition
...
#Region "ApplicationAddInServer Members"
        ...
        Public Sub Activate(ByVal addInSiteObject As Inventor.ApplicationAddInSite, ByVal firstTime As Boolean) Implements Inventor.ApplicationAddInServer.Activate
            ...
            m_timer = New Timer(600000) ' Set timer for 10 minutes (600000 milliseconds)
            ...
            m_settingsButton = g_inventorApplication.CommandManager.ControlDefinitions.AddButtonDefinition("Settings", "SettingsButton", CommandTypesEnum.kQueryOnlyCmdType)

            '' Add the button to the ribbon
            'Dim partRibbon As Ribbon = g_inventorApplication.UserInterfaceManager.Ribbons.Item("Part")
            'Dim tab As RibbonTab = partRibbon.RibbonTabs.Item("id_TabTools")
            'Dim panel As RibbonPanel = tab.RibbonPanels.Add("Settings", "SettingsPanel", "id_PanelSettings")
            'panel.CommandControls.AddButton(m_settingsButton, True, True, "", False)
            ...
            m_settingsButton = controlDefs.AddButtonDefinition("AutoSave Settings", "Internal Name", CommandTypesEnum.kShapeEditCmdType, AddInClientID)
...
#Region "User interface definition"
        ...
        Private Sub AddToUserInterface()
            ' Iterate over all ribbons and add the settings button

            For Each ribbon As Ribbon In g_inventorApplication.UserInterfaceManager.Ribbons
                Try
                    Dim toolsTab As RibbonTab = ribbon.RibbonTabs.Item("id_TabTools")
                    If toolsTab IsNot Nothing Then
                        Dim customPanel As RibbonPanel = toolsTab.RibbonPanels.Add("Lake Monsters", "LMButtons", AddInClientID)
                        customPanel.CommandControls.AddButton(m_settingsButton) ', True, True, "", False)
                    End If
        ...
        Private Sub m_timer_Elapsed(sender As Object, e As ElapsedEventArgs) Handles m_timer.Elapsed
            ' Ensure a document is open before attempting to save
            If g_inventorApplication.Documents.Count > 0 Then
                ...
                If doc.FullFileName <> "" Then
                    'doc.Save()
                    MsgBox("AutoSave " & doc.FullFileName)
                Else
        ...
        Private Sub m_sampleButton_OnExecute(Context As NameValueMap) Handles m_settingsButton.OnExecute
...
#Region "Function to get the add-in client ID."
    ...
    Public Function AddInClientID() As String
        Dim guid As String = ""
        Try
            Dim t As Type = GetType(InventorAutoSaveAddIn.StandardAddInServer)
